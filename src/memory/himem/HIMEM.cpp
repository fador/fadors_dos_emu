...
void HIMEM::freePool(uint32_t offset, uint32_t size) {
    ...
    while (it != m_freeBlocks.end() && it->offset < offset) ++it;
...
bool HIMEM::enableA20() {
    m_a20Enabled = true;
    if (m_memBus) m_memBus->setA20(true);
    return true;
}

bool HIMEM::disableA20() {
    m_a20Enabled = false;
    if (m_memBus) m_memBus->setA20(false);
    return true;
}
...
bool HIMEM::requestHMA(uint16_t /*size*/) {
    if (m_hmaAllocated) { m_lastError = ERR_HMA_IN_USE; return false; }
    m_hmaAllocated = true;
    return true;
}

bool HIMEM::releaseHMA() {
    if (!m_hmaAllocated) { m_lastError = ERR_HMA_NOT_ALLOC; return false; }
...
uint16_t HIMEM::allocateEMB(uint16_t sizeKB) {
    ...
    if (sizeBytes == 0) sizeBytes = 1024; // minimum 1KB
...
bool HIMEM::resizeEMB(uint16_t handle, uint16_t newSizeKB) {
    ...
    std::memcpy(&m_xms[newOffset], &m_xms[oldOffset], copySize);
...
bool HIMEM::moveEMB(uint32_t length,
                    uint16_t srcHandle, uint32_t srcOffset,
                    uint16_t dstHandle, uint32_t dstOffset,
                    MemoryBus& memory) {
    // Handle 0 means conventional memory (srcOffset/dstOffset is seg:off linear addr)
    auto getPtr = [&](uint16_t h, uint32_t off, uint32_t len, bool /*isWrite*/) -> uint8_t* {
        if (h == 0) {
        ...
        return &m_xms[m_handles[h].offset + off];
    };
...
uint16_t HIMEM::queryFreeKB(uint16_t& largestBlockKB) const {
    ...
    largestBlockKB = static_cast<uint16_t>(std::min(largest / 1024, (uint32_t)0xFFFF));
    return static_cast<uint16_t>(std::min(total / 1024, (uint32_t)0xFFFF));
}

uint8_t* HIMEM::getBlock(uint16_t handle) {
    if (handle == 0 || handle >= m_handles.size() || !m_handles[handle].allocated) return nullptr;
    return &m_xms[m_handles[handle].offset];
}
...
uint32_t HIMEM::available() const {
    uint32_t total = 0;
    for (const auto& fb : m_freeBlocks) total += fb.size;
    return total;
}

uint16_t HIMEM::allocate(uint32_t size) {
    uint16_t sizeKB = static_cast<uint16_t>(std::min((size + 1023) / 1024, (uint32_t)0xFFFF));
    return allocateEMB(sizeKB);
}

bool HIMEM::free(uint16_t handle) {
    return freeEMB(handle);
}
