...
public:
  ...
  uint32_t getHandleSizeKB(uint16_t handle) const;
  uint8_t getLockCount(uint16_t handle) const;
  uint8_t getFreeHandles() const;

  // EMB Move (XMS function 0Bh)
  // The move struct is read from DOS memory by the caller.
  bool moveEMB(uint32_t length, uint16_t srcHandle, uint32_t srcOffset,
